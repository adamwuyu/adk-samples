...
from draft_craft.agent import root_agent
...
async def test_llm_tools_flow():
    ...
    initial_message.parts = [initial_part]
    ...
    async for event in runner.run_async(
        ...
        all_initial_events.append(event)
        # 可以添加处理每个事件的逻辑
    ...
    follow_up_part = types.Part()
    follow_up_part.text = "请开始为我生成初稿"
    follow_up_message.parts = [follow_up_part]
    ...
    async for event in runner.run_async(
        ...
        all_followup_events.append(event)
        # 可以添加处理每个事件的逻辑
    ...
    final_part.text = "请取回最终文稿结果"
    final_message.parts = [final_part]
